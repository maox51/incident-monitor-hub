...
-- Política para la vista de consolidados detallados
CREATE POLICY "Usuarios pueden ver vista consolidados detallados de sus áreas"
ON public.vista_consolidados_detallados
FOR SELECT
USING (
  -- Admins y supervisores de monitoreo pueden ver todo
  has_role(auth.uid(), 'admin'::app_role) OR 
  ...
  EXISTS (
    SELECT 1 
    FROM public.incidencias i
    WHERE DATE(i.fecha_incidencia) = vista_consolidados_detallados.fecha_reporte
    AND i.estado = 'aprobado'
    AND (
      has_role(auth.uid(), 'monitor'::app_role) OR
      (
...
ALTER TABLE public.vista_consolidados_detallados ENABLE ROW LEVEL SECURITY;