-- Actualizar función obtener_consolidado_con_medios para incluir nombres de usuarios
...
BEGIN
  ...
  FROM (
      ...
      COALESCE(
        json_agg(
          json_build_object(
            ...
            'area', a.nombre,
            'sala', s.nombre,
            'reportado_por', COALESCE(p.full_name, i.reportado_por),
    ...
    LEFT JOIN public.profiles p ON i.reportado_por = p.id::text
    LEFT JOIN (
      SELECT 
        incidencia_id,
        COUNT(*) as total
...
-- Actualizar el trigger para usar el nuevo edge function de FCM
